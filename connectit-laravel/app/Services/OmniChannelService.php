...
class OmniChannelService
{
    ...
    public function sendEmail(
        ...
        try {
            // Track thread
            $threadId = $metadata['thread_id'] ?? $ticket->ticket_number;
            \App\Models\EmailThread::updateOrCreate(
                ['thread_id' => $threadId],
                ['ticket_id' => $ticket->id, 'subject' => $subjectWithTicket, 'last_message_at' => now()]
            );
            ...
            // Log to EmailLog (enterprise requirement)
    ...
    public function notifySLABreached(Ticket $ticket, string $slaType): void
    {
        $subject = "🚨 SLA BREACH: [{$ticket->ticket_number}]";
        $body = "SLA Breach alert for ticket #{$ticket->ticket_number}.\nType: {$slaType}\nPriority: {$ticket->priority->value}";
        
        // Notify management/agent
        $recipient = config('mail.from.address');
        \App\Jobs\SendEmailNotification::dispatch($ticket, $recipient, $subject, $body);

        $this->createInAppNotifications(
            $this->staffRecipients($ticket),
            'sla_breach',
            'SLA Breached',
            "Ticket {$ticket->ticket_number} has breached its {$slaType} SLA.",
            $ticket
        );
    }
    ...
    {
        // 1. Try to match by Ticket Number in Subject (most reliable)
        $haystacks = [
            (string) ($payload['subject'] ?? ''),
        ];

        foreach ($haystacks as $text) {
            if (preg_match('/\b(?:INC|TK)[-#\s]?(\d{7})\b/i', $text, $matches)) {
                $ticketNumber = 'INC' . $matches[1];
                $ticket = Ticket::where('ticket_number', $ticketNumber)->first();
                if ($ticket) return $ticket;
            }
        }

        // 2. Try to match by Thread ID in EmailThreads table
        if (!empty($payload['thread_id'])) {
            $thread = \App\Models\EmailThread::where('thread_id', $payload['thread_id'])->first();
            if ($thread) return $thread->ticket;
        }

        // 3. Try to match by message_id in EmailMessages (if it's a reply)
        if (!empty($payload['thread_id'])) {
            $message = EmailMessage::where('message_id', $payload['thread_id'])->first();
            if ($message?->ticket_id) return Ticket::find($message->ticket_id);
        }

        // 4. Try to match by Ticket Number in Body (fallback)
        if (preg_match('/\b(?:INC|TK)[-#\s]?(\d{7})\b/i', (string)($payload['body_text'] ?? ''), $matches)) {
            $ticketNumber = 'INC' . $matches[1];
            $ticket = Ticket::where('ticket_number', $ticketNumber)->first();
            if ($ticket) return $ticket;
        }
